...
| 状态 | 数量 | 条目 |
|------|------|------|
| ✅ 已修复 | 11 | 发现 #1-#10 全部、许可证一致性（技术债项） |
| 🟡 部分修复 | 1 | 私有成员访问收敛（技术债项） |
| ❌ 未修复 | 1 | main_window 单类行数（A5，需独立重构计划） |

修复率约 **93%**（14/15 完成）。**发现 #1-#10 全部修复**；剩余仅 main_window 拆分的结构性技术债（需独立重构计划，不宜混入缺陷修复轮）。审查 12 天间项目经历了 v1.2.x 打包重建与 v1.3.0 音量功能，但**运行时/安全类问题基本未动**。
...
> 2026-08-29 更新（第二批）：**#5 目录穿越**（`69a4ec2` 消毒 + resolve 守卫）、**#8 缩略图**（`69a4ec2` 项目目录解析 + 越界拒绝）、**#6 指针竞态**（`9cf7f7d` 锁 + 快照 + normalize）、**#7 BGR/RGB**（`f1c316b`）、**#9 GPU 异步探测**（`f1c316b`）、**#10 日志**（`f1c316b`）全部修复；`_apply_atempo` 死代码删除；`create_project` 定性为待接线 API（保留，消毒已完成）——共 +21 回归测试，测试规模 655 → 691。
> 2026-09-01 更新（复验）：**发现 #1-#10 全部修复**，逐项对照当前代码复核通过（详见上文各节）。`test_concurrent_write_read_snapshots_never_raise` 的跨快照恒等断言（`len(frames) == len(meta_ts)` 等）在写入线程活跃时不可满足——不同属性的快照取自不同时刻，只保证单调不减（前缀关系），已修正断言并 15 次循环验证稳定，全量 691 通过。
